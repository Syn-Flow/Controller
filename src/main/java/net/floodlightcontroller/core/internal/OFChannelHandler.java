...
import edu.messagetype.OFXPortModReply;
import edu.messagetype.OFXTableFeature;
...
class OFChannelHandler
    ...
    enum ChannelState {
        ...
        MASTER(true) {
            ...
            @Override
            void processOFXFlowModReply(OFChannelHandler h,
            		OFXFlowModReply m) throws IOException {
            	h.dispatchMessage(m);
            }
            @Override
            void processOFXPortModReply(OFChannelHandler h,
            		OFXPortModReply m) throws IOException {
            	h.dispatchMessage(m);
            }
            @Override
            void processOFXTableFeature(OFChannelHandler h,
            		OFXTableFeature m) throws IOException {
            	h.dispatchMessage(m);
            }
        },
        ...
        void processOFMessage(OFChannelHandler h, OFMessage m) throws IOException {
            ...
            switch(m.getType()) {
                ...
                case OFX_FLOW_MOD_REPLY:
                	processOFXFlowModReply(h,(OFXFlowModReply)m);
                	break;
                case OFX_PORT_MOD_REPLY:
                	processOFXPortModReply(h,(OFXPortModReply)m);
                	break;
                case OFX_TABLE_FEATURE:
                	processOFXTableFeature(h,(OFXTableFeature)m);
                	break;
        ...
        void processOFXFlowModReply(OFChannelHandler h, OFXFlowModReply m)
				throws IOException {
        	unhandledMessageReceived(h, m);			
		}
        void processOFXPortModReply(OFChannelHandler h, OFXPortModReply m)
        		throws IOException {
        	unhandledMessageReceived(h, m);
        }
        void processOFXTableFeature(OFChannelHandler h, OFXTableFeature m)
        		throws IOException {
        	unhandledMessageReceived(h, m);
        }
    }
